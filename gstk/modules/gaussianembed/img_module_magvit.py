...
class Encoder(nn.Module):
    def __init__(self, *, ch, img_ch, z_channels, num_res_blocks, ch_mult=(1, 2, 2, 4)):
        super().__init__()

        self.num_res_blocks = num_res_blocks
        self.num_blocks = len(ch_mult)
        
        self.conv_in = nn.Conv2d(img_ch, ch, kernel_size=(3, 3), padding=1, bias=False)
        ...
        for i_level in range(self.num_blocks):
            down = nn.Module()
            block = nn.ModuleList()
            block_out = ch*ch_mult[i_level] # 128*[1, 2, 2, 4]
            for _ in range(self.num_res_blocks):
                block.append(ResBlock(block_in, block_out))
                block_in = block_out
            
            down.block = block
            if i_level < self.num_blocks - 1: # num_blocks-1个downsample
    ...
    def forward(self, x):
        ...
        for i_mdie_res in range(self.num_res_blocks):
            x = self.mid_block[i_mdie_res](x)
...
class Decoder(nn.Module):
    def __init__(self, *, ch, img_ch, z_channels, num_res_blocks, ch_mult=(1, 2, 2, 4)):
        ...
        for i_level in reversed(range(self.num_blocks)):
            ...
            up.block = block
            if i_level > 0:
                up.upsample = Upsampler(block_out)
            self.up.insert(0, up)
        
        self.norm_out = nn.GroupNorm(32, block_out, eps=1e-6)
        self.conv_out = nn.Conv2d(block_out, img_ch, kernel_size=(3, 3), padding=1)
    ...
    def forward(self, z):
        ...
        for i_level in reversed(range(self.num_blocks)):
            ...
            if i_level > 0:
                z = self.up[i_level].upsample(z) # i_level=0时不upsample
